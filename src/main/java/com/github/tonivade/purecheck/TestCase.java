...
public interface TestCase<E, T> extends TestCaseOf<E, T> {
   ...
   *
   * @param <E> type of the error generated by the validator
   * @param <T> type of the input value of the operation
   * @param <R> type of the result returned by the operation
   * @param name name of the test case
   * @return a new test case
   */
  static <E, T, R> GivenStep<E, T, R> test(String name) {
    return given -> when -> then -> new TestCaseImpl<>(name, IO.suspend(() -> when.apply(given)), then);
  }

  /**
   * It describes the given value used in this test
   *
   * @param <E> type of the error generated by the validator
   * @param <T> type of the input value of the operation
   * @param <R> type of the result returned by the operation
   */
  interface GivenStep<E, T, R> {

    WhenStep<E, T, R> given(T value);

    default WhenStep<E, T, R> skip() {
      return given(null);
    }
  }
   ...
   * @param <R> type of the result returned by the operation
   */
  interface WhenStep<E, T, R> {

    ThenStep<E, R> run(Function1<T, IO<R>> when);

    default ThenStep<E, R> when(Function1<T, R> when) {
      return run(when.liftTry().andThen(result -> result.fold(IO::raiseError, IO::pure)));
    }

    default ThenStep<E, R> when(IO<R> when) {
      return run(ignore -> when);
    }

    default ThenStep<E, R> when(Producer<R> when) {
      return when(IO.task(when));
    }

    default ThenStep<E, R> error(Throwable error) {
   ...
   * @param <R> type of the result returned by the operation
   */
  interface ThenStep<E, R> {

    TestCase<E, R> then(Either<Validator<Result<E>, Throwable>, Validator<Result<E>, R>> validator);

    default TestCase<E, R> onSuccess(Validator<Result<E>, R> validator) {
      return then(Either.right(validator));
    }

    default TestCase<E, R> onFailure(Validator<Result<E>, Throwable> validator) {
      return then(Either.left(validator));
    }

    default TestCase<E, R> thenCheck(Validator<E, R> validator) {
      return onSuccess(validator.mapError(Result::of));
    }

    default TestCase<E, R> thenError(Validator<E, Throwable> validator) {
      return onFailure(validator.mapError(Result::of));
    }
  }
}

class TestCaseImpl<E, T> implements SealedTestCase<E, T> {
